...
class DashboardController extends Controller
{
    public function index()
    {
        $pemesanan = Pemesanan::select(
            DB::raw("SUM( IF( status = 'pesan',1,0 ) ) as jumlah_permintaan"),
            DB::raw("SUM( IF( status = 'checkin',1,0 ) ) as jumlah_checkin")
        )->first();
        ...
        $fasilitas = FasilitasHotel::select(
            DB::raw('count(id) as jumlah_fasilitas')
        )->first();
    ...
    public function data_chart()
    {
        $pemesanan = Pemesanan::select(
            'created_at',
            DB::raw('count(*) as jumlah_pemesanan')
        )
        ...
        foreach($pemesanan as $row){
            ...
            $data['data'][] = $row->jumlah_pemesanan;
        }
